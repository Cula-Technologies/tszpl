import { Barcode, BarcodeType, BarcodeTypeName, Spacing } from '../src/index.js';
import { createLabel } from './test-helpers.js';

const testHelpers = { createLabel };

test('add QR barcode to a label', async () => {
...
test('add DataMatrix barcode to a label', async () => {
  const label = testHelpers.createLabel();
...
test('add EAN13 barcode to a label', async () => {
  const label = testHelpers.createLabel();
...
test('add EAN13 barcode without an interpretation line to a label', async () => {
  const label = testHelpers.createLabel();
...
test('add EAN8 barcode to a label', async () => {
  const label = testHelpers.createLabel();
